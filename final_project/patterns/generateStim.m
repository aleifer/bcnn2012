...
PATTERN_WHITE_LEVEL = 1-2*whiteFraction; % -1 = white to +1 = black
...
try
    ...
    SCREEN_WIDTH = screenRect(3); %pixels
    SCREEN_HEIGHT = screenRect(4); %pixels
    texsize = SCREEN_HEIGHT/2; % half the size of a square displaying pattern
    upper_left = [SCREEN_WIDTH/4 SCREEN_HEIGHT/4];
    xcenter = upper_left(1)+SCREEN_WIDTH/2*(xcenter-1);
    ycenter = upper_left(2)+SCREEN_HEIGHT/2*(ycenter-1);
    gaussian_sigma = SCREEN_HEIGHT/3.1;
    ...
    grating=(black+white)/2 + inc*(-1).^(cos(fr*x)<PATTERN_WHITE_LEVEL);
    ...
    mask(:, :, 2)=white * ((1 - exp(-((x/gaussian_sigma).^2)-((y/gaussian_sigma).^2)))>0.5);
